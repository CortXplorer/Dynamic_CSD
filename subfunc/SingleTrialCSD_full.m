function [SingleRec_preAVREC,Rec_preAVREC,AvgFP, SingleTrialFP, AvgCSD,...
    ...
    SingleTrialAbsResCSD, SingleLayerRelRes, AvgLayerRelRes, numart] =...
...
for i_trial = 1:size(stim_list,2)
    ...
    % Artifact Correction 
    numart = 0;
    if max(max(abs(x1))) >= 0.9 % far too big
        x1 = NaN(size(x1));  % replace full trial with NaNs
        numart = numart + 1;
    end
...
for i_stim = un_stim
    ...
    AvgLayerRelRes{i_stim} = nanmean(dummy_LayerRelRes,3);
    ...
    Rec_preAVREC{i_stim} = nanmean(dummy_SingleTrialLayerAVREC,3 );
    clear dummy_SingleTrialAbsResCSD
end
